...
scale=10500/(4010+10500);
Jrelease=200000*scale;  % calcium released via a single IPR channel

% constant IPR parameters
q45=0;q54=3330;q12=1240;q21=0;q23=0;q32=69;q26=0;q62=4010;

Vs=Jrelease*20; % rate of cytoplasmic removal from the cluster
Ks=12000; % half-saturation constant for cytoplasmic removal from the cluster
% sl=500;  % step length

Jleak=Vs*100/(100+Ks); % Ca2+ influx from cluster environment
...
cm_nPast=c(end)*ones(Num_IPR,nPast); % past calcium concentrations are assumed to be constant
...
dis=100; % discretisation parameter
...
while time(end)<Tmax
    ...
    for i=1:Num_IPR
        ...
        cm(i)=c_new+120000*heaviside(3-state(i,end)); % heaviside function changed for reduced model
...
%     pars(2)=No;
    ...
    time_c(end+1)=time_c(end)+dt;
 
    %% tracking the gating variable solutions
    h24=h24_new;
    h24_track(:,end+1)=h24_new;

    m24=m24_new;
    m24_track(:,end+1)=m24_new;

    h42=h42_new;
    h42_track(:,end+1)=h42_new;
   
    m42=m42_new;
    m42_track(:,end+1)=m42_new;
    ...
    if index==0
        ...
        q24=(a24+V24*(1-m24(index).*h24(index)))*(1-scale);
...
save(['puff_10IPRs_IP3_01uM__reduced2s_',num2str(full)])
...
load(['puff_10IPRs_IP3_01uM_reduced2s_',num2str(ldf)])
...
save('puff_10IPRs_IP3_01uM_reduced2s_400s')
